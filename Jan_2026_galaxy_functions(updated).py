...
def Detected_Galaxies(Element, data_table, waveMin, waveMax, sensitivity, tobsv, fov, area):
    ...
    for multi_elem in Element:
        print(f"Processing element: {multi_elem}")
        line_index,waveRest=ElementInput(multi_elem)
     ...
     #area = 1.96 #in square degrees
        ...
        Df = pd.DataFrame({"catIndices": catIndices})

        dataframe_all.append(Df)
        dash_list.append(catIndices)
        
    return dataframe_all, dash_list
